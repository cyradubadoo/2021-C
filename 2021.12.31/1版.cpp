...
int main()
{
	...
	//查询储存图书信息的文件是否存在
	if((fp=fopen("d:\\boookbook.txt","rb+"))==NULL)
	{
		printf("在d盘根目录下没有找到储存图书信息的boookbook.txt文件\n请选择  1--手动导入！  2--创建此文件\n");
		int choice;
		scanf("%d",&choice);
		switch(choice)
		{
		case 1:printf("请把名为boookbook.txt的文件复制到d盘根目录下\n\n");break;
		
		case 2:if((fp=fopen("d:\\boookbook.txt","ab+"))!=NULL)
				   printf("创建成功\n\n");
...
int check_repeat_book(char num[30])
{
    ...
    fp=fopen("d:\\boookbook.txt","r");
   	booknumber=0;
    while(!feof(fp))
    {
        fscanf(fp,"%s %s %s %s %s \n",
				bookinfo[booknumber].num,bookinfo[booknumber].name,bookinfo[booknumber].writer,
				bookinfo[booknumber].press,bookinfo[booknumber].category);
       	booknumber++;
    }
    fclose(fp);
	for(n=0;n<booknumber;n++)
    	if(strcmp(bookinfo[n].num,num)==0)
    	{
    	    flag1=1;
			break;
		}

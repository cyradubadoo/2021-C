...
//声明所用函数
void Menu(bo book[]);//菜单
void Input(bo book[]);//录入	
void Save(bo book[]);//保存
void Browse(bo book[]);//浏览
void Search(bo book[]);//查询
void Add(bo book[]);//增加
void Delete(bo book[]);//删除
void Revise(bo book[]);//修改
void Statistics(bo book[]);//统计
void Sort(bo book[]);//排序
void Quit(bo book[]);//退出

int check_repeat(char num[30]);//查重书籍序号
int check_lenth(bo book[]);
...
void Browse(bo book[])                                           
{
	...
	{	
		rewind(fp);
		//while(!feof(fp))
		while(fscanf(fp,"%s %s %s %s %s\n",book[booknumber].num,book[booknumber].name,book[booknumber].writer,book[booknumber].press,book[booknumber].category)!=EOF)
		{
      	//	fscanf(fp,"%s %s %s %s %s\n",book[booknumber].num,book[booknumber].name,book[booknumber].writer,book[booknumber].press,book[booknumber].category);
      		booknumber++;	
		}
		...
		for(n=0;n<len;n++)//while(n<=booknumber)
		{
...
void Quit(bo book[])
{
	...
	if(choice=='N'||choice=='n') printf("返回菜单"); system("pause");
	 Menu(book);//跳转至菜单
}
